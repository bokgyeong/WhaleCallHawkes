...
// Decay function
// [[Rcpp::export]]
double compH(double tdiff, double sdiff, double soundspeed, double eta, double phi){
  return exp(-eta * (tdiff - sdiff / soundspeed)) * exp(-phi * sdiff);
}


// Integral of decay function
// [[Rcpp::export]]
vec compIntHvec(vec tdiff, double sdiff, double soundspeed, double eta, double phi){
  return  exp(-phi * sdiff) * (1 - exp(-eta * (tdiff - sdiff / soundspeed))) / eta;
}


// Integral of decay function
// [[Rcpp::export]]
double compIntH(double tdiff, double sdiff, double soundspeed, double eta, double phi){
  return  exp(-phi * sdiff) * (1 - exp(-eta * (tdiff - sdiff / soundspeed))) / eta;
}


// [[Rcpp::export]]
double compSumIntHl(int l, vec ts, vec marks, double maxT, mat distmat, double soundspeed, double eta, double phi){
  ...
  for(int i = 0; i < nl; i ++){
    for(int k = 0; k < K; k ++){
      dummyrate(i,k) = compIntH(maxT - tsl[i], distmat(l, k), soundspeed, eta, phi);
    }
  }
...
// [[Rcpp::export]]
double compSumIntHl_parallel(int l, vec ts, vec marks, double maxT, mat distmat, double soundspeed, double eta, double phi, int ncores){
  uvec dummy = find( (marks == l) );
  vec tsl = ts.elem( dummy );
  int nl = tsl.size(), K = distmat.n_rows;
  mat dummyrate = zeros(nl, K);
...
// [[Rcpp::export]]
double compSumIntH(vec ts, vec marks, double maxT, mat distmat, double soundspeed, vec zeta, double eta, double phi){
  int n = ts.size(), K = distmat.n_cols;
  mat dummyrate = zeros(n, K);
  
  for(int i = 0; i < n; i ++){
    for(int k = 0; k < K; k ++){
      dummyrate(i,k) = zeta[ marks[i] ] * compIntH(maxT - ts[i], distmat(marks[i], k), soundspeed, eta, phi);
    }
  }
...
// [[Rcpp::export]]
double compSumIntH_parallel(vec ts, vec marks, double maxT, mat distmat, double soundspeed, vec zeta, double eta, double phi, int ncores){
  int n = ts.size(), K = distmat.n_cols;
  mat dummyrate = zeros(n, K);
...
// [[Rcpp::export]]
double rtctAlphaSumIntH(double tsi, int mi, vec ts, vec marks, mat distmat, double soundspeed, vec alpha, double eta, double phi){
  int n = ts.size();
  double res = 0;
  
  vec distvec = zeros(n);
  for(int ll = 0; ll < n; ll++){
    distvec[ll] = distmat(mi, marks[ll]);
  }
  uvec ind = find( ts + (distvec / soundspeed) < tsi );
  int sizeind = ind.size();
  
  for(int j = 0; j < sizeind; j ++){
    res = res + alpha[ marks[ ind[j] ] ] * compIntH(tsi - ts[ ind[j] ], distmat(mi, marks[ ind[j] ]), soundspeed, eta, phi);
  }
...
// [[Rcpp::export]]
mat rtctAlphaSumIntHik(double tsi, vec ts, vec marks, mat distmat, double soundspeed, mat alpha, vec eta, vec phi){
  ...
  for(int k = 0; k < K; k ++){
    for(int iter = 0; iter < niters; iter ++){
      ...
      etak = eta[iter];
      phik = phi[iter];
      intTrigi(iter,k) = rtctAlphaSumIntH(tsi, k, ts, marks, distmat, soundspeed, alphak, etak, phik);
    }
  }
...
// [[Rcpp::export]]
double compLogLiki(vec ts, vec marks, mat distmat, double soundspeed, double maxT, mat lam0m, vec indlam0, vec knts, vec alpha, double eta, double phi){
  ...
  for(int i = 0; i < n; i ++){
    ...
    if( sum(alpha) == 0 ){
      ...
      vec distvec = zeros(n);
      for(int ll = 0; ll < n; ll++){
        distvec[ll] = distmat(marks[i], marks[ll]);
      }
      
      uvec ind = find( ts + (distvec / soundspeed) < ts[i] );
      sizeind = ind.size();
      
      vec ahi = zeros(sizeind);
      for(int j = 0; j < sizeind; j ++){
        ahi[j] = alpha[marks[j]] * compH(ts[i] - ts[j], distmat(marks[i], marks[j]), soundspeed, eta, phi);
      }
      ...
      for(int k = 0; k < K; k ++){
        IntH[k] = compIntH(maxT - ts[i], distmat(k, marks[i]), soundspeed, eta, phi);
      }
...
// [[Rcpp::export]]
List compLamt(double t, vec ts, vec marks, mat distmat, double soundspeed, vec knts, double maxT, double indlam0t, 
              List Xm, mat beta0, List beta, mat delta, mat Wm, mat alpha, vec eta, vec phi){
  
  int niters = beta0.n_rows, m = Wm.n_cols, K = distmat.n_rows, sizeind, n = ts.size();
  vec lam0m;
  mat Xmk, betak;
  ...
  for(int k = 0; k < K; k ++){
    for(int iter = 0; iter < niters; iter ++){
      ...
      vec distvec = zeros(n);
      for(int ll = 0; ll < n; ll++){
        distvec[ll] = distmat(k, marks[ll]);
      }
      
      uvec ind = find( ts + (distvec / soundspeed) < t );
      sizeind = ind.size();
      
      vec ahi = zeros(sizeind);
      for(int j = 0; j < sizeind; j ++){
        ahi[j] = alphaiter[ marks[j] ] * compH(t - ts[j], distmat(k, marks[j]), soundspeed, etaiter, phiiter);
...
// [[Rcpp::export]]
vec sampleBranching(vec ts, vec marks, mat distmat, double soundspeed, vec lam0, vec zeta, double eta, double phi) {
  int n = ts.size(), l, k, parent, ndummy;
  double temp;
  vec branching = zeros(n), probs, distvec = zeros(n);
  uvec dummy;
  ...
  for (int i = 1; i < n; i++) {
    ...
    dummy = find( ts + (distvec / soundspeed) < ts[i] ); // sound speed
    ndummy = dummy.size();
    ...
    if(ndummy == 0){
      ...
      for (int j = 0; j < ndummy; j++) {
        l = marks[ dummy[j] ];
        temp = zeta[l] * compH(ts[i] - ts[ dummy[j] ], distmat(l,k), soundspeed, eta, phi);
        probs[j+1] = temp;
      }
...
List fitLGCP(int niter, vec ts, vec marks, List Xm, double maxT,
             ...
             vec Wm, double logdeltaTilde, vec logdelta, vec indlam0,
             double shape_tau, double rate_tau,
             vec sigma2, mat COVbeta0, List COVbeta, mat COVlogdelta,
             bool updateCOV, int adaptInterval, double adaptFactorExponent,
             vec adapIter, int ncores) {
  ...
  int n = ts.size(), p = beta.n_rows, K = distmat.n_rows, m = knts.size() - 1;
  double logprob;
  mat postBeta0 = zeros(niter, K), postWm(niter, (m+1)), postLogdelta(niter, K), postTau = zeros(niter, 1+p+1), postTilde = zeros(niter,p+2);
  ...
  mat cholCOVbeta0(K, K), cholCOVlogdelta(K, K);
  List cholCOVbeta(p);
  cholCOVbeta0 = trans( chol( sigma2[0] * ( COVbeta0 + 0.0000000001 * diagmat(ones(K)) ) ) );
  for(int i = 0; i < p; i ++){
    cholCOVbeta[i] = trans( chol( sigma2[1+i] * ( as<mat>(COVbeta[i]) + 0.0000000001 * diagmat(ones(K)) ) ) );
  }
  cholCOVlogdelta = trans( chol( sigma2[1+p] * ( COVlogdelta + 0.0000000001 * diagmat(ones(K)) ) ) );
  ...
  vec lam0, newlam0;
  if(ncores == 1){
    intLam0 = compIntLam0(maxT, beta0, Xm, beta, exp(logdelta), Wm);
    lam0 = compLam0(ts, marks, maxT, beta0, Xm, beta, exp(logdelta), Wm, indlam0, knts);
    
  } else {
    intLam0 = compIntLam0_parallel(maxT, beta0, Xm, beta, exp(logdelta), Wm, ncores);
    lam0 = compLam0_parallel(ts, marks, maxT, beta0, Xm, beta, exp(logdelta), Wm, indlam0, knts, ncores);
  }
  ...
  for(int s = 0; s < niter; s++) {
    ...
    newbeta0 = beta0 + cholCOVbeta0 * randn(K);
    if(ncores == 1){
      newlam0 = compLam0(ts, marks, maxT, newbeta0, Xm, beta, exp(logdelta), Wm, indlam0, knts);
      newintLam0 = compIntLam0(maxT, newbeta0, Xm, beta, exp(logdelta), Wm);
      
    } else {
      newlam0 = compLam0_parallel(ts, marks, maxT, newbeta0, Xm, beta, exp(logdelta), Wm, indlam0, knts, ncores);
      newintLam0 = compIntLam0_parallel(maxT, newbeta0, Xm, beta, exp(logdelta), Wm, ncores);
    }
    ...
    for(int i = 0; i < p; i ++){
      ...
      if(ncores == 1){
        newlam0 = compLam0(ts, marks, maxT, beta0, Xm, newbeta, exp(logdelta), Wm, indlam0, knts);
        newintLam0 = compIntLam0(maxT, beta0, Xm, newbeta, exp(logdelta), Wm);
        
      } else {
        newlam0 = compLam0_parallel(ts, marks, maxT, beta0, Xm, newbeta, exp(logdelta), Wm, indlam0, knts, ncores);
        newintLam0 = compIntLam0_parallel(maxT, beta0, Xm, newbeta, exp(logdelta), Wm, ncores);
      }
    ...
    while(accept == false){
      ...
      if(ncores == 1){
        newlam0 = compLam0(ts, marks, maxT, beta0, Xm, beta, exp(logdelta), newWm, indlam0, knts);
        newintLam0 = compIntLam0(maxT, beta0, Xm, beta, exp(logdelta), newWm);
        
      } else {
        newlam0 = compLam0_parallel(ts, marks, maxT, beta0, Xm, beta, exp(logdelta), newWm, indlam0, knts, ncores);
        newintLam0 = compIntLam0_parallel(maxT, beta0, Xm, beta, exp(logdelta), newWm, ncores);
      }
    ...
    if( updateCOV ){
      if( (s+1 >= adaptInterval) && (s+1 - (adaptInterval * trunc((s+1) / adaptInterval)) == 0) ){
        ...
        COVlogdelta = COVlogdelta + gamma1[1+p] * ( cov( postLogdelta.rows(s+1-adaptInterval, s-1) ) - COVlogdelta );
        cholCOVlogdelta = trans( chol( sigma2[1+p] * ( COVlogdelta + 0.0000000001 * diagmat(ones(K)) ) ) );
    ...
    newlogdelta = logdelta + cholCOVlogdelta * randn(K);
    if(ncores == 1){
      newlam0 = compLam0(ts, marks, maxT, beta0, Xm, beta, exp(newlogdelta), Wm, indlam0, knts);
      newintLam0 = compIntLam0(maxT, beta0, Xm, beta, exp(newlogdelta), Wm);
      
    } else {
      newlam0 = compLam0_parallel(ts, marks, maxT, beta0, Xm, beta, exp(newlogdelta), Wm, indlam0, knts, ncores);
      newintLam0 = compIntLam0_parallel(maxT, beta0, Xm, beta, exp(newlogdelta), Wm, ncores);
    }
    
    logprob = (sum(log(newlam0)) - sum(log(lam0))) - (newintLam0 - intLam0) +
      MVN_logh(newlogdelta, logdeltaTilde * ones(K), invVmat/tau[1+p]) - MVN_logh(logdelta, logdeltaTilde * ones(K), invVmat/tau[1+p]);
    
    if (log(randu()) < logprob) {
      logdelta = newlogdelta;
      lam0 = newlam0;
      intLam0 = newintLam0;
    ...
    postLogdelta.row(s) = trans(logdelta);
    if(s == 0){ Rprintf("sampled logdelta\n"); }
    
    
    
    // Gibbs update for logdeltaTilde
    sTilde = 1 / (oneinvVmatone / tau[1+p] + 1/100);
    mTilde = sTilde / tau[1+p] * sum(trans(logdelta) * invVmatone);
    logdeltaTilde = mTilde + sqrt(sTilde) * randn();
    postTilde(s,1+p) = logdeltaTilde;
    if(s == 0){ Rprintf("sampled logdeltaTilde\n"); }
    ...
    shape = shape_tau + 0.5 * K;
    scale = ( 1 / ( rate_tau + 0.5 * trans(logdelta - logdeltaTilde) * invVmat * (logdelta - logdeltaTilde) ) )[0];
  ...
  return Rcpp::List::create(Rcpp::Named("postBeta0") = postBeta0,
                            Rcpp::Named("postBeta") = postBeta,
                            Rcpp::Named("postTau") = postTau,
                            Rcpp::Named("postLogdelta") = postLogdelta,
                            Rcpp::Named("postTilde") = postTilde,
                            ...
                            Rcpp::Named("COVbeta0") = COVbeta0,
                            Rcpp::Named("COVbeta") = COVbeta,
                            Rcpp::Named("COVlogdelta") = COVlogdelta);
}
...
// [[Rcpp::export]]
List fitNHPPSE(int niter, vec ts, vec marks, List Xm, double maxT, double soundspeed,
               double rho_beta,
  ...
  for(int s = 0; s < niter; s++) {
    ...
    branching = sampleBranching(ts, marks, distmat, soundspeed, lam0, zeta, eta, phi);
    ...
    for(int l = 0; l < K; l ++){
      ...
      if(ncores == 1){
        dummySum = compSumIntHl(l, ts, marks, maxT, distmat, soundspeed, eta, phi);
      } else{
        dummySum = compSumIntHl_parallel(l, ts, marks, maxT, distmat, soundspeed, eta, phi, ncores);
      }
    ...
    if( (neweta < lb_eta) || (neweta > ub_eta) ){
      ...
      if(ncores == 1){
        dummynewSum = compSumIntH(ts, marks, maxT, distmat, soundspeed, zeta, neweta, phi);
        dummySum = compSumIntH(ts, marks, maxT, distmat, soundspeed, zeta, eta, phi);
      } else{
        dummynewSum = compSumIntH_parallel(ts, marks, maxT, distmat, soundspeed, zeta, neweta, phi, ncores);
        dummySum = compSumIntH_parallel(ts, marks, maxT, distmat, soundspeed, zeta, eta, phi, ncores);
      }
    ...
    if( (newphi < lb_phi) || (newphi > ub_phi) ){
      ...
      if(ncores == 1){
        dummynewSum = compSumIntH(ts, marks, maxT, distmat, soundspeed, zeta, eta, newphi);
        dummySum = compSumIntH(ts, marks, maxT, distmat, soundspeed, zeta, eta, phi);
      } else{
        dummynewSum = compSumIntH_parallel(ts, marks, maxT, distmat, soundspeed, zeta, eta, newphi, ncores);
        dummySum = compSumIntH_parallel(ts, marks, maxT, distmat, soundspeed, zeta, eta, phi, ncores);
      }
...
// ESS for W, fixed rho, variance = 1, mean = 0
// MVN prior for beta0, beta, and logdelta
// Gamma prior for zeta and logdelta
// Unif prior for eta and phi
// [[Rcpp::export]]
List fitLGCPSE(int niter, vec ts, vec marks, List Xm, double maxT, double soundspeed,
               double rho_beta, double rho_w,
               mat distmat, vec knts, mat tdiffm,
               double betaTilde0, vec beta0, vec betaTilde, mat beta, vec tau,
               vec Wm, double logdeltaTilde, vec logdelta, vec indlam0,
               ...
               double lb_phi, double ub_phi,
               vec sigma2, mat COVbeta0, List COVbeta,
               mat COVlogdelta, double COVeta, double COVphi,
               bool updateCOV, int adaptInterval, double adaptFactorExponent,
               vec adapIter, int ncores) {
  ...
  int n = ts.size(), p = beta.n_rows, K = distmat.n_rows, m = knts.size() - 1;
  double logprob;
  mat postBeta0 = zeros(niter, K), postWm(niter, (m+1)), postLogdelta(niter, K);
...
// computing time
// [[Rcpp::export]]
void comLGCPSE(int niter, vec ts, vec marks, List Xm, double maxT, double soundspeed,
               double rho_beta, double rho_w,
               mat distmat, vec knts, mat tdiffm,
               double betaTilde0, vec beta0, vec betaTilde, mat beta, vec tau,
               vec Wm, double logdeltaTilde, vec logdelta, vec indlam0,
