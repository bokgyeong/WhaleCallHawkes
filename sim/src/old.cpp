...
// [[Rcpp::export]]
double compSumIntHl(int l, vec ts, vec marks, double maxT, mat distmat, double eta, double phi, double soundspeed){
  ...
  for(int i = 0; i < nl; i ++){
    for(int k = 0; k < K; k ++){
      dummyrate(i,k) = compIntH(maxT - tsl[i] - distmat(l, k) / soundspeed, distmat(l, k), eta, phi);
    }
  }
...
// [[Rcpp::export]]
double compSumIntHl_parallel(int l, vec ts, vec marks, double maxT, mat distmat, double eta, double phi, double soundspeed, int ncores){
  uvec dummy = find( (marks == l) );
  vec tsl = ts.elem( dummy );
  int nl = tsl.size(), K = distmat.n_rows;
  mat dummyrate = zeros(nl, K);
...
// [[Rcpp::export]]
double compSumIntH(vec ts, vec marks, double maxT, mat distmat, vec zeta, double eta, double phi, double soundspeed){
  int n = ts.size(), K = distmat.n_cols;
  mat dummyrate = zeros(n, K);
  
  for(int i = 0; i < n; i ++){
    for(int k = 0; k < K; k ++){
      dummyrate(i,k) = zeta[ marks[i] ] * compIntH(maxT - ts[i] - distmat(marks[i], k) / soundspeed, distmat(marks[i], k), eta, phi);
    }
  }
...
// [[Rcpp::export]]
double compSumIntH_parallel(vec ts, vec marks, double maxT, mat distmat, vec zeta, double eta, double phi, double soundspeed, int ncores){
  int n = ts.size(), K = distmat.n_cols;
  mat dummyrate = zeros(n, K);
...
// [[Rcpp::export]]
double rtctAlphaSumIntH(double tsi, int mi, vec ts, vec marks, mat distmat, vec alpha, double eta, double phi, double soundspeed){
  int n = ts.size();
  double res = 0;
  vec distvec = zeros(n);
  ...
  for(int j = 0; j < sizeind; j ++){
    res = res + alpha[ marks[ ind[j] ] ] * compIntH(tsi - ts[ ind[j] ] - distmat(mi, marks[ ind[j] ]) / soundspeed, distmat(mi, marks[ ind[j] ]), eta, phi);
  }
...
// [[Rcpp::export]]
mat rtctAlphaSumIntHik(double tsi, vec ts, vec marks, mat distmat, mat alpha, vec eta, vec phi, double soundspeed){
  ...
  for(int k = 0; k < K; k ++){
    for(int iter = 0; iter < niters; iter ++){
      ...
      etak = eta[iter];
      phik = phi[iter];
      intTrigi(iter,k) = rtctAlphaSumIntH(tsi, k, ts, marks, distmat, alphak, etak, phik, soundspeed);
    }
  }
...
// [[Rcpp::export]]
double compLogLiki(vec ts, vec marks, mat distmat, double maxT, mat lam0m, vec indlam0, vec knts, vec alpha, double eta, double phi, double soundspeed){
  ...
  for(int i = 0; i < n; i ++){
    ...
    if( sum(alpha) == 0 ){
      ...
      for(int j = 0; j < sizeind; j ++){
        ahi[j] = alpha[marks[j]] * exp( -eta * (ts[i] - ts[j] - distmat(marks[i], marks[j]) / soundspeed)) * exp( -phi * distmat(marks[i], marks[j]));
      }
      ...
      for(int k = 0; k < K; k ++){
        IntH[k] = compIntH(maxT - ts[i] - distmat(k, marks[i]) / soundspeed, distmat(k, marks[i]), eta, phi);
      }
...
List compLamt(double t, vec ts, vec marks, mat distmat, vec knts, double maxT, double indlam0t, 
              List Xm, mat beta0, List beta, mat delta, mat Wm, mat alpha, vec eta, vec phi, double soundspeed){
  ...
  for(int k = 0; k < K; k ++){
    for(int iter = 0; iter < niters; iter ++){
      ...
      vec ahi = zeros(sizeind);
      for(int j = 0; j < sizeind; j ++){
        ahi[j] = alphaiter[ marks[j] ] * exp( -etaiter * (t - ts[j] - distmat(k, marks[j]) / soundspeed)) * exp( -phiiter * distmat(k, marks[j]));
      }
      sek(iter,k) = sum(ahi);  
    }
  ...
  for (int i = 1; i < n; i++) {
    ...
    if(ndummy == 0){
      ...
      for (int j = 0; j < ndummy; j++) {
        l = marks[ dummy[j] ];
        temp = zeta[l] * compH(ts[i] - ts[ dummy[j] ] - distmat(l,k) / soundspeed, distmat(l,k), eta, phi);
        probs[j+1] = temp;
      }
...
List fitNHPPSE(int niter, vec ts, vec marks, List Xm, double maxT, double soundspeed,
  ...
  for(int s = 0; s < niter; s++) {
    ...
    for(int l = 0; l < K; l ++){
      ...
      if(ncores == 1){
        dummySum = compSumIntHl(l, ts, marks, maxT, distmat, eta, phi, soundspeed);
      } else{
        dummySum = compSumIntHl_parallel(l, ts, marks, maxT, distmat, eta, phi, soundspeed, ncores);
      }
    ...
    if( (neweta < lb_eta) || (neweta > ub_eta) ){
      ...
      if(ncores == 1){
        dummynewSum = compSumIntH(ts, marks, maxT, distmat, zeta, neweta, phi, soundspeed);
        dummySum = compSumIntH(ts, marks, maxT, distmat, zeta, eta, phi, soundspeed);
      } else{
        dummynewSum = compSumIntH_parallel(ts, marks, maxT, distmat, zeta, neweta, phi, soundspeed, ncores);
        dummySum = compSumIntH_parallel(ts, marks, maxT, distmat, zeta, eta, phi, soundspeed, ncores);
      }
    ...
    if( (newphi < lb_phi) || (newphi > ub_phi) ){
      ...
      if(ncores == 1){
        dummynewSum = compSumIntH(ts, marks, maxT, distmat, zeta, eta, newphi, soundspeed);
        dummySum = compSumIntH(ts, marks, maxT, distmat, zeta, eta, phi, soundspeed);
      } else{
        dummynewSum = compSumIntH_parallel(ts, marks, maxT, distmat, zeta, eta, newphi, soundspeed, ncores);
        dummySum = compSumIntH_parallel(ts, marks, maxT, distmat, zeta, eta, phi, soundspeed, ncores);
      }
