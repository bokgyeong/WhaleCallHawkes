...
for(aa in (start+1):length(aaa) ){
  ...
  dummy = foreach(iter = (aaa[aa-1]+1):(aaa[aa]), .combine = c, .packages = 'Rcpp') %do% {
    lam0m = foreach(k = 1:K, .combine = cbind) %do% { exp( postBeta0[iter,k] + Xm[[k]] %*% postBetanew[[k]][iter,] + exp(postLogdelta[iter,k]) * postWm[iter,] ) }
    compLogLiki(ts, marks, distmat, soundspeed, maxT, lam0m, indlam0, knts, rep(0, K), 1, 1)
  }
