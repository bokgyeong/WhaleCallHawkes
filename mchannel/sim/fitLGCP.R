...
filename = paste0('sim/fit/sim', datasets[runID], 'fitLGCP.RData')
...
start = 1; postBeta0 = postTau = postDelta = postTilde = postWm = c(); postBeta = sapply(1:p, function(i) c(), simplify = F); Accprob = 0; rtime = 0
# load(filename); start = which(outers == nrow(postBeta0))

# i = 2; outers = seq(0, 100, by = 100); adaptInterval = 25
# i = 2; outers = seq(0, 1000, by = 500); adaptInterval = 100
# sourceCpp('src/test.cpp')
...
for(i in (start+1):length(outers) ){
  ...
  dummy = fitLGCP(
    ...
    deltaTilde, delta, indlam0, shape_tau, rate_tau, sigma2, 
    COVbeta0, COVbeta, COVdelta, updateCOV, adaptInterval, adaptFactorExponent, 
    adapIter, ncores) 
  ...
  save(sback, rtime, postBeta0, postBeta, postTilde, postWm, postTau,
       postDelta, Accprob, betaTilde0, beta0, betaTilde, beta, tau, deltaTilde, delta, 
       Wm, rho_beta, rho_w, shape_tau, rate_tau,
       sigma2, adapIter, COVbeta0, COVbeta, COVdelta,
