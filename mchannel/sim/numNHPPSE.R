...
load(paste0('sim/fit/sim', datasets[runID], 'fitNHPPSE.RData'))
filename = paste0('sim/num/sim', datasets[runID], 'fitNHPPSEnum.RData')
...
for(aa in (start+1):length(aaa) ){
  ...
  for(iter in (aaa[aa-1]+1):(aaa[aa])){
    ...
    numBack = sapply(1:K, function(k) compIntLam0k(maxT, postBeta0[iter,k], Xm[[k]], postBetanew[[k]][iter,], 0, rep(0, nrow(Xm[[k]]))))
