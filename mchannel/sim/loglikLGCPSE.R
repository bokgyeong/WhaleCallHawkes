...
filename = paste0('sim/loglik/sim', datasets[runID], 'fitLGCPSEloglik.RData')
...
for(aa in (start+1):length(aaa) ){
  ...
  for(iter in (aaa[aa-1]+1):(aaa[aa])){
    ...
    dummy = c(dummy, compLogLiki(ts, marks, distmat, maxT, lam0m, indlam0, knts, postZeta[iter,], postEta[iter], postPhi[iter]))
  }
