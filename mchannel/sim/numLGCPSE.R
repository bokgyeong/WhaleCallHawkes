...
load(paste0('sim/fit/sim', datasets[runID], 'fitLGCPSE.RData'))
filename = paste0('sim/num/sim', datasets[runID], 'fitLGCPSEnum.RData')
...
for(aa in (start+1):length(aaa) ){
  ...
  for(iter in (aaa[aa-1]+1):(aaa[aa])){
    ...
    numBack = sapply(1:K, function(k) compIntLam0k(maxT, postBeta0[iter,k], Xm[[k]], postBetanew[[k]][iter,], exp(postDelta[iter,k]), postWm[iter,]))
    
    numSE = matrix(0, K, K)
    for(l in 1:K){
      for(k in 1:K){
        tsk = ts[ marks == (k-1) ]
        numSE[l,k] = sum( postZeta[iter,l] / postEta[iter] * (1 - exp( - postEta[iter] * (maxT - tsk) )) * exp( - postPhi[iter] * distmat[l,k] ) )
      }
    }

    dummy = rbind(dummy, c(numBack, as.matrix(numSE, byrow = T)))
  }
