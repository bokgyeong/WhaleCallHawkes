...
simulateTrig = function(l, intensity_ftn, maxintensity, maxT, distmat){
  points = c()
  ...
  for(k in 1:K){
    ...
    while(TRUE){
      ...
      if(runif(1) < intensity_ftn(l, newTime, distmat[l,k]) / maxintensity){
        points = rbind(points, c(newTime, k))
      }
    }
...
simulateMultiselfex = function(Xm, distmat, maxT, lam0m, knts, 
                            beta0, beta, Wm = NULL, delta = NULL, zeta, eta, phi,
                            displayOutput = TRUE){
  ...
  if( all(zeta == 0) ) {
    ...
    while (TRUE) {
      ...
      fn_trig = function(l, tdiff, sdiff) {
        return( zeta[l] * compH(tdiff, sdiff, eta, phi)  )
      }
      
      pts = simulateTrig(pt[2], fn_trig, maxtrig[pt[2]], maxT - pt[1], distmat)
      if( !is.null(pts) ){
