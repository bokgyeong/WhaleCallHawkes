...
library(xtable); library(suncalc)
...
dummy = getSunlightTimes(date = seq.Date(as.Date('2010-04-02'), as.Date('2010-04-10'), by = 1),
...
shade = data.frame(
  ...
  dawn = as.POSIXlt(c(as.character(dummy$sunrise), '2010-04-11 00:00:00'), tz = 'UTC', format = '%Y-%m-%d %H:%M:%S'),
  top = Inf,
  bottom = -Inf
)
