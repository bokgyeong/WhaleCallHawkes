...
load(paste0('nopp/fit/noppNHPP.RData'))
filename = paste0('nopp/loglik/noppNHPPloglik.RData')
...
for(aa in (start+1):length(aaa) ){
  ...
  for(iter in (aaa[aa-1]+1):(aaa[aa])){
    ...
    dummy = c(dummy, compLogLiki(ts, maxT, lam0m, indlam0, knts, 0, 1))
  }
